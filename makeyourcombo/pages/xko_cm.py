...
from makeyourcombo.components.navbar import game_navbar, xko_mobile_navbar
from makeyourcombo.components.xko_input_buttons import *
...
class XkoCharacterState(rx.State):
    selected_character: str = "Ahri"
    image_path: str = "/2xko/characters/ahri.webp"
...
class XkoComboState(rx.State):
    ...
    def merge_images_and_download(self, combo_name: str):
        ...
        font_path = rx.get_upload_dir() / "2xko/fonts/Roboto-MediumItalic.ttf"
        custom_font_size = 24
        ...
        if combo_name:  # Add the background image only if combo_name has text
            bg_image_path = bg_image_path = rx.get_upload_dir() / "2xko/inputs/background.png"
...
def character_menu(character_dict):
    menu_items = [
        rx.menu.item(
            character,
            on_select=XkoCharacterState.select_character(character, image_path)
        )
    ...
    return rx.vstack(
        rx.menu.root(
            rx.menu.trigger(
                rx.button("Select Character", margin_y=["1em", "1.5em"], background="#cdf564", color="#1c1d1f"),
            ),
            rx.menu.content(
                *menu_items
            ),
...
@rx.page(route="/2xko", title="Make your Combo | 2XKO Combo Maker")
def xko_page() -> rx.Component:
...
rx.box(
    rx.center(
        rx.image(src=XkoCharacterState.image_path, border_radius="1.2em"),
        width="100%",
    ),
    #padding_x="1em",
    width=["100%"],
    ...
    justify="center",
            ...
            rx.hstack(
                ...
                rx.hstack(
                    rx.input(value=XkoComboState.combo_name,on_change=XkoComboState.set_combo_name,placeholder="Combo Name(Wall Combo, etc.)",min_width="10rem", width="100%", float="right", font_size=["0.6rem","0.8rem"], background="#1c1d1f"),
                    rx.button("Clear Combo", on_click=XkoComboState.set_current_combo([]), background="#cdf564", color="#1c1d1f", width="30%"),
                    rx.button("Delete Last Input", on_click=XkoComboState.remove_move(XkoComboState.current_combo), background="#cdf564", color="#1c1d1f", width="30%"),
                    rx.button("Download Combo", on_click=XkoComboState.merge_images_and_download(XkoComboState.combo_name), background="#cdf564", color="#1c1d1f", width="30%"),
            ...
            rx.mobile_only(
                rx.vstack(rx.input(placeholder="Combo Name(Wall Combo, etc.)", width="100%", float="right", font_size="0.6rem", background="#1c1d1f", color="#1c1d1f", min_width="10rem"),
                    rx.button("Clear Combo", on_click=XkoComboState.set_current_combo([]), background="#cdf564",color="#1c1d1f", width="50%"),
                    rx.button("Delete Last Input", on_click=XkoComboState.remove_move(XkoComboState.current_combo), background="#cdf564",color="#1c1d1f", width="50%"),
                    rx.button("Download Combo", on_click=XkoComboState.merge_images_and_download(XkoComboState.combo_name), background="#cdf564",color="#1c1d1f", width="50%"),
                    character_menu(character_dict),
                    align_items= "center",
                    justify_content="space-between",
                    min_width="20rem",
            ...
            rx.vstack(
                ...
                rx.hstack(
                    rx.foreach(
                    XkoComboState.directional_inputs,
                    lambda m: rx.chakra.button(rx.image(src=rx.get_upload_url(m[1]), width="2em",height="auto", margin_x="0.2em",),variant="unstyled", on_click=XkoComboState.add_move(m[0]))
                ),
                width="100%",
                height="auto",
                #padding_left="1.3em",
                ...
                rx.hstack(
                    rx.foreach(
                    XkoComboState.normal_button_inputs,
                    lambda m: rx.chakra.button(rx.image(src=rx.get_upload_url(m[1]), width="2em",height="auto", margin_x="0.2em",),variant="unstyled", on_click=XkoComboState.add_move(m[0]))
                ),
                width="100%",
                height="auto",
                ...
                rx.hstack(
                    rx.foreach(
                        XkoComboState.current_combo,
                        lambda m: rx.image(src=rx.get_upload_url(XkoComboState.all_inputs[m]), width=["1.5rem", "2rem"], border_radius="0.5em"),
                    ),
                max_width="68rem",
                width="100%",
                padding="1rem",
...
character_dict: dict = {
        "Ahri": "/2xko/characters/ahri.webp",
        "Darius": "/2xko/characters/darius.webp",
        "Ekko": "/2xko/characters/ekko.webp",
        "Illaoi": "/2xko/characters/illaoi.webp",
        "Yasuo": "/2xko/characters/yasuo.webp",
    }
